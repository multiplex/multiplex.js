(function (global, factory) {
    ...
    qtest('basic "distinct" test', function (assert) {
        assert.equal(mx__default['default'](array).distinct().count(), 5, ' distinct count of an array of separate numbers');
        assert.equal(mx__default['default']([1, 1, 1]).distinct().count(), 1, ' distinct count of an array of identical numbers');
        assert.equal(mx__default['default'].repeat(1, 100).distinct().count(), 1, ' distinct count of an array of 100 identical numbers');
        assert.equal(mx__default['default'](['1', '1', '1']).distinct().count(), 1, ' distinct count of an array of identical strings');
        assert.equal(mx__default['default']([true, true, true]).distinct().count(), 1, ' distinct count of an array of identical booleans');
        assert.equal(mx__default['default']([null, null, null]).distinct().count(), 1, ' distinct count of an array of null values');
        assert.equal(mx__default['default']([new Date(2017, 0, 1), new Date(2017, 0, 1), new Date(2017, 0, 1)]).distinct().count(), 1, ' distinct count of an array of identical dates');
        assert.equal(mx__default['default']([{ val: 1 }, { val: 1 }, { val: 1 }]).distinct().count(), 1, ' distinct count of an array of identical object literals');
        assert.equal(mx__default['default']([]).distinct().count(), 0, 'distinct count of an empty array');
    });


    qtest('"distinct" test using comparer', function (assert) {
        assert.equal(mx__default['default']([{ val: 1, name: 'A' }, { val: 1, name: 'B' }, { val: 1, name: 'C' }]).distinct({
            hash: function (t) {
                return t.val;
            },
            equals: function (a, b) {
    ...
    qtest('collections "distinct" method tests', function (assert) {
        ...
        assert.equal(enumerable.distinct().count(), 5, 'Test "distinct" in an enumerable');
        assert.equal(enumerable.distinct(numericComparer).count(), 5, 'Test "distinct" in an enumerable with comparer');
