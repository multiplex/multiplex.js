...
qtest('basic "join" tests', function (assert) {
    assert.equal(mx(numbers).join(mocks.array, identity, identity, identity).count(), mocks.array.length, ' join count of an array of separate numbers');
    assert.equal(mx(mocks.array).join(numbers, identity, identity, identity).count(), mocks.array.length, ' join count of an array of separate numbers');
    assert.deepEqual(mx(mocks.array).join(numbers, identity, identity, identity).toArray(), mocks.array, ' join result of an array of separate numbers');

    assert.equal(mx([1, 1, 1]).join(mocks.array, identity, identity, identity).count(), 3, ' join count of an array of identical numbers');
    assert.equal(mx(mocks.array).join([1, 1, 1], identity, identity, identity).count(), 3, ' join count of an array of identical numbers');
    ...
    assert.equal(mx([]).join(mocks.array, identity, identity, identity).count(), 0, ' join count of an empty array');
    assert.equal(mx(mocks.array).join([], identity, identity, identity).count(), 0, ' join count of an empty array');
    assert.equal(mx(mocks.array).join([null], identity, identity, identity).count(), 0, ' join count of an null array');
    assert.equal(mx(mocks.array).join([undefined], identity, identity, identity).count(), 0, ' join count of an undefined array');
